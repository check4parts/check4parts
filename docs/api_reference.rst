...
Space endpoints provide a single entry point for supplier-agnostic search and
product detail lookups. They normalise supplier payloads to the BM Parts shape
(``part``, ``rests``, ``supplier``) and always return a ``meta`` block noting
which suppliers were contacted and whether any failed.

If the request omits ``suppliers``, all supported suppliers are queried. A
partially failing supplier does not stop aggregation; the ``meta`` block
returns the failing suppliers with their status code and error detail.
...
.. list-table::
   ...
   * - POST
     ...
     - ``products_space``
   * - POST
     - ``/space/credentials/{supplier}``
     - ``store_supplier_credentials``
   * - GET
     - ``/space/credentials``
     - ``list_supplier_credentials``

The unified routes only attempt suppliers for which the authenticated client has
stored valid credentials. Environment variables for each adapter are used as
defaults when present, but per-client credentials can be set with the
``/space/credentials/{supplier}`` endpoint.
...
.. code-block:: http
   ...
   POST /space/search HTTP/1.1
   Authorization: Bearer <supabase-jwt>
   Content-Type: application/json

   {
     "query": "oil filter",
     "suppliers": ["bm-parts", "asg"],
     "supplier_options": {
       "asg": {"per_page": 10}
     }
   }
     ...
     "products": [
       {
         ...
         "rests": [
           {"quantity": 7, "price": 10.0, "delivery_time": null}
         ],
         "supplier": "bm-parts",
         "raw": {"id": "123", "name": "Oil Filter"}
       }
     ],
     "meta": {
       "requested_suppliers": ["bm-parts", "asg"],
       "attempted_suppliers": ["bm-parts"],
       "skipped_suppliers": [
         {"supplier": "asg", "reason": "missing_credentials"}
       ],
       "failed_suppliers": [
         {"supplier": "asg", "error": {"status_code": 504, "detail": {"message": "timeout"}}}
       ],
...
.. code-block:: http
   ...
   POST /space/products HTTP/1.1
   Authorization: Bearer <supabase-jwt>
   Content-Type: application/json

   {
     "products": [
       {"supplier": "omega", "product_id": "A123"},
       {"supplier": "bm-parts", "product_id": "B456", "options": {"include_additional": true}}
     ]
   }
...
.. code-block:: json
   ...
   {
     "products": [
       {"part": {"uuid": "A123", "name": "Item", "supplier": "omega"}, "rests": [], "supplier": "omega", "raw": {"id": "A123"}},
       {"part": {"uuid": "B456", "name": "Item", "supplier": "bm-parts"}, "rests": [], "supplier": "bm-parts", "raw": {"id": "B456"}}
     ],
     "meta": {
       "failed_suppliers": [],
